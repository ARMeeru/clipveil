...
fn positives() -> Vec<(&'static str, String, &'static str)> {
    vec![
        ...
        (
            ...
            asm(&[
                "github_pat_",
                "11ABCDEFGHIJ0123456789_abcdefghijKLMNOPQRSTUV",
            ]),
            "github_pat",
        ),
        (
            "gitlab pat",
            asm(&["glpat-", "ABCDEFghij0123456789"]),
            "gitlab_pat",
        ),
        (
            "openai sk-",
            format!("key {}", asm(&["sk-", "abcdefghijklmnopqrstuvwx0123"])),
            "openai_key",
        ),
        (
            "openai sk-proj",
            asm(&["sk-proj-", "abcdefghijklmnop0123456789"]),
            "openai_key",
        ),
        (
            "stripe sk_live",
            asm(&["sk_live_", "0123456789abcdefABCDEF01"]),
            "stripe_key",
        ),
        (
            "stripe rk_live",
            asm(&["rk_live_", "0123456789abcdefABCDEF01"]),
            "stripe_key",
        ),
        (
            "aws AKIA",
            format!("aws_key={}", asm(&["AKIA", "IOSFODNN7EXAMPLE"])),
            "aws_access_key",
        ),
        (
            "aws ASIA",
            asm(&["ASIA", "Y34FZKBOKMUTVV7A"]),
            "aws_access_key",
        ),
        (
            "google api key",
            asm(&["AIza", "SyD-ExampleExampleExampleExample123"]),
            "google_api_key",
        ),
        (
            "google oauth",
        ...
        (
            "slack bot xoxb",
            asm(&["xoxb-", "1234567890-abcdefFGHIJ12345"]),
            "slack_token",
        ),
        (
            "slack app xapp",
        ...
        (
            ...
            format!(
                "x\n{}\nMIIBOwIBAAJBAK\nQ2F0\n{}\ny",
                asm(&["-----BEGIN RSA ", "PRIVATE KEY-----"]),
                asm(&["-----END RSA ", "PRIVATE KEY-----"])
            ),
            "private_key",
        ),
    ]
...
fn negatives() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        (
            "git sha",
            "commit 9cb24bcf1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d done",
        ),
        ("ip address", "server 192.168.1.100 port 8080"),
        (
            "prose token",
            "a token of trust and the secret of life is love",
        ),
...
fn positives_are_detected_and_redacted() {
    for (name, text, kind) in positives() {
        ...
        assert!(
            ...
            "[{name}] expected {kind}, got {found:?}"
        );
        assert!(
            redact(&text).contains("[REDACTED:"),
            "[{name}] redact produced no marker"
        );
    }
}
